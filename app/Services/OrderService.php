...
class OrderService
{
    ...
    public function createOrderFromCart(
        ...
        return DB::transaction(function () use ($cart, $stripePaymentIntentId, $stripePaymentMethod) {
            ...
            $order = Order::create(array_merge([
                ...
                'email' => $cart->email,
                'prefix_phone' => $cart->prefix_phone,
                'phone' => $cart->phone,
                'address' => $cart->address,
                'city' => $cart->city,
                'zip_code' => $cart->zip_code,
                'country_id' => $cart->country_id,
                'fiscal_code' => $cart->fiscal_code,
                'birth_date' => $cart->birth_date,
                'privacy_accepted' => (bool) $cart->privacy_accepted,
                'newsletter' => (bool) $cart->newsletter,
            ...
            $this->snapshotCartConsents($cart, $order);
    ...
    /**
     * Snapshot per-ordine dei consensi accettati nel carrello: una riga per
     * partner_consent_id (versione), immutabile una volta scritta. Il consenso
     * puntato può essere superseded in futuro senza intaccare questo record.
     */
    protected function snapshotCartConsents(Cart $cart, Order $order): void
    {
    ...
    {
        try {
            $order->loadMissing(['partner', 'country', 'orderProducts.product.category', 'orderProducts.items.variant']);
            if ($order->email) {
                Mail::to($order->email)->send(new OrderConfirmationMail($order));
                $this->logger->logEmailSent($order, $order->email);
            }
        } catch (\Throwable $e) {
            report($e);
        }
    ...
    public function createOrderManually(array $payload): Order
    {
        ...
        return DB::transaction(function () use ($partner, $product, $payload, $itemsResolved, $total, $variation, $slot, $totalQuantity) {
            $customerData = $this->normalizeManualCustomerData($payload['customer'] ?? []);
    ...
    /**
     * Normalizza il payload anagrafico dell'ordine manuale sui campi
     * corrispondenti su orders. I dati sono uno snapshot immutabile:
     * ordini precedenti con la stessa email non vengono toccati.
     */
    protected function normalizeManualCustomerData(array $data): array
    {
        $fields = [
            'name' => $data['name'] ?? null,
